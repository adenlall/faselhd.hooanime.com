...
export default async function page({ params }) {
  ...
  let res, data;
  res = await fetch(process.env.URL+"api/item/"+params.path, { next: { revalidate: Number(process.env.REVALIDATE) } });
  data = await res.json();
  ...
  return (
    <article className="pb-2 bg-base-300 text-base-content">
      ...
      <div className="container px-2 m-auto w-full">
        <a href="#episodes" className="btn btn-base btn-outline btn-block btn-lg"><h1>{data?.data?.name}</h1></a>
      </div>
            ...
            <div className="flex flex-wrap justify-center container m-auto my-14 px-2 w-full">
                {
                    data?.data?.parent?.seasons?.map((item, i)=>(
                      <div className="m-2 rounded-box group" style={{background: "url('"+(item?.img ??"")+"')", backgroundSize:"cover", backgroundPosition:"center"}}>
                      <Link style={{"--tw-bg-opacity":0.75}} href={"/seasons"+item?.url} className="p-4 h-40 w-80 flex items-center justify-center group-hover:bg-primary bg-secondary rounded-box">
                        <div className="text-secondary-content group-hover:text-primary-content text-2xl font-black">{item.name}</div>
                      </Link>
                      </div>
                  ))
                }
      ...
      <EpisodeCardTracker
        data={{
          "url":"/anime-episodes/"+params.path,
          "name":data?.data?.name,
...
export async function generateMetadata({ params }) {
  ...
  try {
      ...
      let res, data;
      res = await fetch(process.env.URL+"api/item/"+params.id, { next: { revalidate: Number(process.env.REVALIDATE) } });
      data = await res.json();
      ...
      let res, data;
      res = await fetch(process.env.URL+"api/anime/"+params.id, { next: { revalidate: Number(process.env.REVALIDATE) } });
      data = await res.json();
